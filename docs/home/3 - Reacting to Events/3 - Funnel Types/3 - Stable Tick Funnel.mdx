...
1. **Reacting timely**: Reacting to events in a timely manner is difficult if L2 batches are in-frequent (remember, most L2s only post batches if there is at least 1 tx to post on the L1). However, you may want your game on a L2 to react quickly to [CDEs](./2%20-%20chain-data-extensions.md) coming from the L1.
...
- *Without emulated blocks*: underlying chain block time
- *With emulated blocks*: fixed time interval

<BaseSwitch className="img-full" style={{ height: 'auto', marginBottom: '48px' }} />

Note that emulated blocks have a many:many relationship with the underlying chain:
...
Note that the emulated block time cannot be too fast, or it would lead to non-determinism. This is because you might *think* there isn't a block in a specific time interval, but really there was an it just hasn't propagated to your node yet. 
...
1. We receieve a block that, timestamp-wise, would go in the next slot. Assuming no rollback, that means there can't be any other new block in this slot, so we generate the emulated block for slot *n* early.
<EmulatedEarly className="img-full" style={{ height: 'auto' }} />
1. `BLOCK_TIME + EMULATED_BLOCKS_MAX_WAIT` passes, so we assume there are no new blocks coming and we create the emulated block for slot *n*.
...
Typically syncing starts with `START_BLOCKHEIGHT`. This means but with emulated blocks you can think of syncing as starting from genesis as it needs to create emulated blocks for the whole history of the chain. This is not just about properly creating the emulated blocks - 
...
Then a block on the underlying chain at time `3` would go into *EB_2*, and one at time 6 would go in in *EB_3*.
...
2. It contains no blocks, use the following algorithm:
    1. Take the emulated block number + the previous block's randomness seed, hash them together `base64(sha256([blockNumber.toString(10), prevSeed].join(',')))`
    1. Using prando, generate 20 random numbers between `START_BLOCKHEIGHT` and the current emulated block number.
    1. For each generated blockheight number, fetch the matching randomness seed from the database.
    1. With these 20 previous random seeds, hash them all together, and thus generating the new emulated block hash.
